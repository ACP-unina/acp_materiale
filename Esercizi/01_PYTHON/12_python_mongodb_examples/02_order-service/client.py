...
def run(port):
    ...
    print("############")
    item_to_search = 'Item - A'
    order_search_result = stub.searchOrders(order_management_pb2.StringMessage(value=item_to_search))
    print(f'searchOrders({item_to_search}) invoked. Result:')
    print
    ...
    id_shipment_to_delete = shipment.id
    
    resp = stub.cancelShipment(order_management_pb2.StringMessage(value=id_shipment_to_delete))
    print("cancelShipment() response: ", resp)
